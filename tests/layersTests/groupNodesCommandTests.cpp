...
#include <core\node.h>
#include <layers\nodeSelection\groupNodesCommand.h>

using namespace phi;

TEST(groupNodesCommand, execute_singleNodeToBeGrouped_doesNotGroup)
{
    //Arrange
    ...
    //Act
    auto groupCmd = groupNodesCommand(nodesToGroup);
    groupCmd.execute();
...
TEST(groupNodesCommand, execute_twoNodesToBeGrouped_groupTheTwoNodes)
{
    //Arrange
    auto sceneRoot = new node();
    auto son1 = new node();
