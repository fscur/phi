...
namespace phi
{
    ...
    inline void buffer::storage(GLsizeiptr size, const void* data, bufferStorageUsage::bufferStorageUsage usage)
    {
        glNamedBufferStorage(id, size, data == nullptr ? NULL : data, usage);
        glError::check();
    }

    inline void buffer::data(GLsizeiptr size, const void* data, bufferDataUsage::bufferDataUsage usage)
    {
